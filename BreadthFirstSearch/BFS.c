...
void Busca_Largura (Grafo *G, int s) {
   ...
   while (!empty(Q)) {
      ...
      Vertice* aux = G->listaAdj[u];
      printf("Dequeue %d \n", u);
      while (aux != NULL)  
      {
         // printf("Aux: %d\n", aux->identificador);
         if (V[aux->identificador].cor == BRANCO)
         {
            printf("Enqueue %d", aux->identificador);
            enqueue(Q, aux->identificador);
            V[aux->identificador].cor = CINZA;
            V[aux->identificador].distance = V[u].distance + 1;
            V[aux->identificador].pai = V[u].identificador;
            printf("\nidentificador: %d, u: %d\n", V[u].identificador, u);
         }
         // printf("\nIndo para o proximo\n");
         aux = aux->proximo;
   ...
   for (u = 0; u < G->V; u++) {
      printf("u = %d, cor: %d, distance: %d, pai: %d\n", u, V[u].cor, V[u].distance, V[u].pai);
   }
   free(V);
}
